	options(scipen=999)
	project_directory <- "/lustre/scratch/sboyane/01_greenbuls/09_phylogeny"
	...
	ref_index <- read.table("ragtag.scaffold_filtered_noZ.fasta.fai", stringsAsFactors=F)
	...
	header <- paste('\tgunzip -cd ', project_directory, '/NC_089140.1_RagTag.recode.vcf.gz   | grep "#" > ', project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
