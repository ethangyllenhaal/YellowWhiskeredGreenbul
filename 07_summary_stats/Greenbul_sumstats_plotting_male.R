...
colnames(stat_adds2) <- c("dxy_East_Central", "fst_East_Central",
                          "pi_East", "pi_Central",
                         "TajD_East", "TajD_Central")
# combine data frames
...
kept_stats <- c("chr", "start", "dxy_East_Central","fst_East_Central", 
                "pi_East", "pi_Central",
                "TajD_East", "TajD_Central")
...
# windows (there are 9,067 windows)
...
population_order <- c("East", "Central")
population_names <- c("East", "Central")

# define population comparisons order and naming conventions
pop_split_order <- c("East_Central")
pop_split_names <- c("East x Central")
...
pdf("_window_DXY_male.pdf", width=10, height=10)
...
# loop for each plot
pdf("_window_FST_male.pdf", width=10, height=4)
par(mfrow=c(length(pop_split_order),1))
par(mar=c(2, 2, 0.1, 0.1))
for(b in 1:length(pop_split_order)) {
  # plot the chromosomes
...
# loop for each plot
pdf("_window_pi_male.pdf", width=10, height=10)
...
# loop for each plot
pdf("_window_TajimaD_male.pdf", width=10, height=10)
par(mfrow=c(length(population_order),1))
par(mar=c(2, 2, 0.1, 0.1))
for(b in 1:length(population_order)) {
  # plot the chromosomes
