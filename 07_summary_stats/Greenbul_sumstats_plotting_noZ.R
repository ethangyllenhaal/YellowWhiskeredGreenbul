...
dxy <- read.table("window_noZ100k_dxy.txt", header=T)
fst <- read.table("window_noZ100k_fst.txt", header=T)
pi <- read.table("window_noZ100k_pi.txt", header=T)
pi$calculated_stat[is.na(pi$calculated_stat)] <- 0
tajima <- read.table("window_noZ100k_tajima.txt", header=T)
# combine all
...
stat_order <- rbind(c("East", "West", "Dxy"),
                    c("East", "Central", "Dxy"),
                    c("Central", "West", "Dxy"),
                    c("East", "West", "Fst"),
                    c("East", "Central", "Fst"),
...
colnames(stat_adds2) <- c("dxy_East_Central", "dxy_Central_West", "dxy_East_West", 
                          "fst_East_Central", "fst_Central_West", "fst_East_West",
...
pop_split_order <- c("East_West", "East_Central", "Central_West")
pop_split_names <- c("East x West", "East x Central", "Central x West")
...
pdf("_window_DXY_noZ.pdf", width=10, height=10)
...
# loop for each plot
pdf("_window_FST_noZ.pdf", width=10, height=10)
par(mfrow=c(length(pop_split_order),1))
par(mar=c(2, 2, 0.1, 0.1))
for(b in 1:length(pop_split_order)) {
  # plot the chromosomes
...
# loop for each plot
pdf("_window_pi_noZ.pdf", width=10, height=10)
...
# loop for each plot
pdf("_window_TajimaD_noZ.pdf", width=10, height=10)
par(mfrow=c(length(population_order),1))
par(mar=c(2, 2, 0.1, 0.1))
for(b in 1:length(population_order)) {
  # plot the chromosomes
